...
GOES-East (GOES-19) ABI L2 Clear Sky Mask, CONUS 2 km every 5 minutes:
NOAA's own binary split (BCM: 0 = clear_or_probably_clear,
1 = cloudy_or_probably_cloudy) gated by DQF==0 (good quality) -> cloud
cover % = observed fraction of cloudy pixels in each canvas neighborhood
(a true area density, no invented precision) -> FULL BASIN RECTANGLE
(lon -93..-73.5, lat 40.5..49.5; only clear <1% and missing data are
transparent) -> spectrum gradient dark-blue (thin) to dark-purple
(overcast) -> PNG -> key image + metadata -> Folder live KML + stable
entry KML (individual files).

Observed satellite retrieval, not a weather model: no analysis blocks,
no assimilation footprints. 100% the source's current information.
...
def run():
    try:
        file_url, scan_dt = goes_acm.newest_file()
    except Exception as e:
        print(f"[{PRODUCT}] NO FILE AVAILABLE (keeping previous): {e}")
        return 2
    source_id = f"goes19-acmc-{scan_dt.strftime('%Y%m%d-%H%M')}"
    ...
    if (prev.get("source_id") == source_id
        ...
        return _refresh_kml()
    try:
        return _build(file_url, scan_dt, source_id)
    except Exception as e:
        traceback.print_exc()
...
def _build(file_url, scan_dt, source_id):
    bounds = load_bounds()
    stage = stage_dir(PRODUCT)
    stage_prod = os.path.join(stage, "site", PRODUCT)
    W, H = bounds["canvas_width"], bounds["canvas_height"]
    raw_path = os.path.join(RAW_DIR, "goes_acmc_current.nc")
    size = goes_acm.download(file_url, raw_path)
    if size < 100_000:
        raise ValueError(f"download too small ({size} bytes)")
    got = goes_acm.read_mask(raw_path)
    cloudy = np.asarray(got["cloudy01"], dtype=float).ravel()
    lats = np.asarray(got["lats"], dtype=float).ravel()
    lons = np.asarray(got["lons"], dtype=float).ravel()

    ok_src = np.isfinite(cloudy) & ((cloudy == 0.0) | (cloudy == 1.0))
    if int(ok_src.sum()) < 5_000:
        raise ValueError(f"too few valid source cells ({int(ok_src.sum())})")
    frac = float(cloudy[ok_src].mean())
    print(f"[{PRODUCT}] GOES-19 ACMC {scan_dt:%Y-%m-%d %H:%MZ}: valid="
          f"{int(ok_src.sum())} basin cloudy fraction={frac:.2f}")

    # Off-disk (space) grid points carry NaN lat/lon: drop them before the
    # canvas mapping (canvas_indices would only warn and ignore them).
    keep = (ok_src & np.isfinite(lats) & np.isfinite(lons))
    rows, cols, valid = canvas_indices(lats[keep], lons[keep], bounds)
    vals = cloudy[keep]
    # 2 km source on a ~1.2 km canvas viewed obliquely: ground spacing
    # stretches to ~3 canvas px with a rotated footprint, so radius 2
    # closes the diamond gaps between splats. Mean of the binary mask IS
    # the observed neighborhood cloud fraction (true area density).
    field01, _counts = bin_to_canvas(rows, cols, vals, valid, (H, W),
                                     splat_radius=2)
    field = field01 * 100.0
    ...
    subtitle = (f"Cloud cover ({unit}, GOES-East 5-min)  |  {data_time_utc}  |  "
                f"basin mean {cur_mean:.0f}%, max {cur_max:.0f}%")
    lw, lh = draw_scale_legend(
        os.path.join(stage_prod, "legend.png"), CONFIG["title"], subtitle,
        "%", CLOUD_STOPS, CLOUD_LABELS,
        f"Source: NOAA GOES-19 ABI clear-sky mask {scan_dt:%Y-%m-%d %H:%MZ}  |  "
        f"Processed {now_det_str()}",
        note="Clear sky (<1%) is transparent, not colored.")
    scale_html = ("Cloud cover (%, OBSERVED neighborhood fraction from "
                  "GOES-East: share of 2 km satellite pixels flagged cloudy "
                  "around each point): <b>LOWEST 0 clear</b> transparent "
                  "&rarr; thin cloud <b>dark-blue/blue/cyan</b> &rarr; "
                  ...
                  "color; clear areas stay see-through. Satellite retrieval, "
                  "not a weather model: no analysis blocks.")
    meta = base_metadata(
        PRODUCT, CONFIG["title"], CONFIG["freshness_label"],
        CONFIG["source_name"], CONFIG["source_url"], CONFIG["variable"],
        data_time_utc=data_time_utc,
        source_last_modified_utc="n/a (S3)",
        units="% (display = observed % cloudy)",
        source_resolution="2 km GOES-19 ABI fixed grid (1500x2500 CONUS), "
                          "binary mask mean-binned to canvas",
        color_min=0.0, color_max=100.0, color_units="%",
        missing_data_treatment=("only DQF-good BCM 0/1 admitted; clear (<1%) "
                                "and missing render fully transparent; full "
                                "basin rectangle, no shoreline cut; never "
                                "zero-filled."))
    meta["legend_size"] = [lw, lh]
    meta["legend_scale_html"] = scale_html
    meta["model_cycle"] = f"GOES-19 scan {scan_dt:%Y-%m-%d %H:%MZ}"
    meta["source_id"] = source_id
    meta["source_version"] = source_token(source_id)
    meta["goes_file"] = file_url.split("/")[-1]
    meta["subpoint_lon"] = got["subpoint_lon"]
    meta["stats"] = {"valid_cells": n_valid, "current_max_pct": cur_max,
                     "current_mean_pct": round(cur_mean, 2),
