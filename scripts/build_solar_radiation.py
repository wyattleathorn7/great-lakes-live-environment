...
def _build(base, datestr, cycle):
    ...
    raw_path = os.path.join(RAW_DIR, "hrrr_solar_current.grib2")
    fetch_messages(base, raw_path, [("DSWRF", "surface")])
    (vals, lats, lons, data_date, data_time) = read_messages(
        raw_path, {"DSWRF": 1})["DSWRF"]
    ...
    field, _c = bin_to_canvas(rows, cols, np.asarray(vals).ravel(),
